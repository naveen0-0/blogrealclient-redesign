...
export default function NewBlog() {
  ...
  const createBlog = (e: React.ChangeEvent<HTMLFormElement>) => {
    ...
    if (files !== null) {
      ...
      axios
        .post(
          "https://api.cloudinary.com/v1_1/ddwkg0hpz/image/upload",
          formData
        )
        .then(async ({ data }) => {
          try {
            let res = await axios.post(
              `${SERVER_BASE_URL}/api/newblog`,
              {
                title,
                description,
                keyword1,
              ...
              {
                headers: {
                  Authorization: localStorage.getItem("blogrealauthtoken")!,
                },
              }
            );
  ...
  return (
    <div className="mx-auto w-[95%] md:w-[80%]">
      <div className="font-anton text-3xl text-center m-3">
        Create A New Blog
      </div>
      ...
      <form onSubmit={createBlog} className="flex flex-col justify-around">
        <label
          htmlFor="file-input"
          className="cursor-pointer my-1 outline-none p-3 font-roboto rounded-sm bg-slate-900 text-slate-300"
        >
        ...
        <button
          disabled={btnDis}
          className="mb-12 flex items-center bg-slate-900 my-1 w-max px-4 py-2 font-roboto rounded-full text-slate-400 hover:text-slate-100"
        >
          <div>Create</div>
          {!btnDis && (
            <div className="ml-2">
