...
export default function FullBlog() {
  ...
  const CommentOnThePost = async (e:React.ChangeEvent<HTMLFormElement>) => {
    e.preventDefault()
    let { data } = await axios.post(`${SERVER_BASE_URL}/api/comment/${id}`,{ comment },{ headers : { Authorization : localStorage.getItem('blogrealauthtoken')!}})
    console.log(data)
    setBlog((prevState:any) => {
      return {
        ...prevState,
        comments:[{ comment : data.comment, username:data.username },...prevState.comments]
      }
    })
    setComment("")
  }
  ...
  return (
    <div className='w-[95%] md:w-[80%] mx-auto bg-slate-600 p-2 my-3 rounded-md shadow-2xl'>
      ...
      <div className='my-1 p-1 font-roboto'>
        {blog.description}
      </div>
      ...
      <div className='my-1'>
        ...
        {loggedIn? 
          <form onSubmit={CommentOnThePost}
          ...
          >
            <input
              ...
              <div className='flex-1 font-roboto text-slate-400 ml-1'>Comment</div>
              </button>
          </form> 
          : 
        ...
        <div>
          {blog.comments.map((comment:any, index:number) => <Comment key={index} comment={comment}/>)}
        </div>
...
const Comment = ({ comment }:{ comment:{comment:string, username:string} }) => {
  return (
    <div className='font-roboto my-1 p-1 rounded-sm'>

      <div className='flex items-center'>
        <div className='text-sm bg-slate-900 w-6  aspect-square rounded-full flex justify-center items-center text-slate-500'>{comment.username[0].toUpperCase()}</div>
